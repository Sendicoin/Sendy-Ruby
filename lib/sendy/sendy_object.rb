...
module Sendy
  class SendyObject
    ...
    # Determines the equality of two Sendy objects. sendy objects are
    # considered to be equal if they have the same set of values and each one
    # of those values is the same.
    def ==(other)
      other.is_a?(SendyObject) && @values == other.instance_variable_get(:@values)
    end
    ...
    def refresh_from(values, partial = false)
      initialize_from(values, partial)
    end
    ...
    #   which applies only to new SendyObjects being initiated under this
    #   SendyObject. Defaults to true.
    ...
    # Sets all keys within the SendyObject as unsaved so that they will be
    # included with an update when #serialize_params is called. This method is
    # also recursive, so any SendyObjects contained as values or which are
    ...
    def serialize_params(options = {})
      ...
      @values.each do |k, v|
        ...
        #   3. Its value is a SendyObject. A sendyObject may contain modified
    ...
    def initialize_from(values, partial = false)
      ...
      update_attributes(values, dirty: false)
    ...
    def serialize_params_value(value, original, unsaved, force, key: nil)
      ...
      # making sure any time one is set, we convert it to a SendyObject. This
      ...
      # existing array being held by a SendyObject. This could happen for
      # example by appending a new hash onto `additional_owners` for an
      # account.
      elsif value.is_a?(Hash)
        Util.convert_to_sendy_object(values).serialize_params

      elsif value.is_a?(SendyObject)
