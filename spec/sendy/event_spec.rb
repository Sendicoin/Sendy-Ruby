...
describe Sendy::Event do
  ...
  it "is listable" do
    ...
    assert_requested :get, "#{Sendy.app_host}/api/v1/events"
  ...
  it "is retrievable" do
    ...
    assert_requested :get, "#{Sendy.app_host}/api/v1/events/1"
    expect(event.is_a?(Sendy::Event))
  end

  it "is creatable" do
    event = Sendy::Event.create
    assert_requested :post, "#{Sendy.app_host}/api/v1/events"
    expect(event.is_a?(Sendy::Event))
  end
  ...
  it "is not saveable" do
    ...
    assert_not_requested :post, "#{Sendy.app_host}/api/v1/events/#{event.id}"
  end
